...
class MultipleAugmentedImagesPage extends StatefulWidget {
  ...
  const MultipleAugmentedImagesPage({super.key, this.path});
  @override
...
class _MultipleAugmentedImagesPageState
  ...
  loadMultipleImage() async {
    if (widget.path != null) {
      final image = File(widget.path!).readAsBytesSync();
      bytesMap["earth_augmented_image"] = image;
      bytesMap["prova_texture"] = image;
      bytesMap["umano_digitale"] = image;
    } else {
